...
This layer answers whether an agent can act through typed interfaces. TDM denotes text and data mining, and its reservation protocol lets sites state mining permissions in machine form \cite{ref20}.
...
rfc-7807-errors & GET \url{/api} probe & Standard problem-details shape on errors & Errors unparseable \\
...
Each domain receives 34 primary probes covering the paths in Section 3, plus up to 6 conditional follow-ups for declared sitemaps, legacy security.txt, well-known llms.txt, OpenAPI YAML, sitemap indexes, and feeds declared in page headers. Probes address the apex origin exactly as listed, with no www-subdomain fallback, so hosts serving only on www read as absent and the limitations record that boundary. The engine fans probes out in parallel with an automatic-abort timeout of 4,500ms per probe. Two homepage replays rotate the User-Agent to GPTBot and PerplexityBot. One replay sets Accept to text/markdown. Two MCP probes POST a JSON-RPC initialize payload to the well-known and API paths \cite{ref8}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\columnwidth]{figures/fig1_adoption.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\columnwidth]{figures/fig2_score_hist.png}
\caption{Distribution of 0 to 100 readiness scores across all 50,000 domains with the mean marked.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\columnwidth]{figures/fig3_tier_compare.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\columnwidth]{figures/fig4_grades_by_tier.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\columnwidth]{figures/fig5_layers_by_tier.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\columnwidth]{figures/fig6_failure_pareto.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\columnwidth]{figures/fig7_policy_behavior.png}
